...
import SafeKRC20NamerTest from '../build/SafeKRC20NamerTest.json'
import FakeCompliantKRC20 from '../build/NamerTestFakeCompliantKRC20.json'
import FakeNoncompliantKRC20 from '../build/NamerTestFakeNoncompliantKRC20.json'
import FakeOptionalKRC20 from '../build/NamerTestFakeOptionalKRC20.json'
...
describe('SafeKRC20Namer', () => {
  ...
  before('deploy SafeKRC20NamerTest', async () => {
    safeNamer = await deployContract(wallet, SafeKRC20NamerTest, [], overrides)
  })

  function deployCompliant({name, symbol}: {name: string; symbol: string}): Promise<Contract> {
    return deployContract(wallet, FakeCompliantKRC20, [name, symbol], overrides)
  }

  function deployNoncompliant({name, symbol}: {name: string; symbol: string}): Promise<Contract> {
    return deployContract(
      wallet,
      FakeNoncompliantKRC20,
  ...
  function deployOptional(): Promise<Contract> {
    return deployContract(wallet, FakeOptionalKRC20, [], overrides)
  }
